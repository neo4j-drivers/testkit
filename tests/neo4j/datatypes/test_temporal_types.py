    ...
    def test_date_time_cypher_created_tz_id(self):
        ...
        for tz_id in TZ_IDS:
            ...
            with self.subTest(tz_id=tz_id):
                ...
                # We are not testing the offset value because the server's and
                # the driver's timezone db may diverge.
        ...
        for (s, dt) in (
            ...
            with self.subTest(s=s, dt=dt):
                ...
                if get_driver_name() == "python":
                    # Python cannot discriminate between +00:00 and +00:00[UTC]
