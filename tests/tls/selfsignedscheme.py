...
class TestSelfSignedScheme(unittest.TestCase):
    """ Tests URL scheme neo4j+ssc/bolt+ssc where server is assumed to present
    a signed server certificate but not necessarily signed by an authority
        ...
        for scheme in schemes:
            with self.subTest(scheme):
                ...
                self._server = TlsServer(
                        "untrustedRoot_thehost", disableTls=True)
