...
class Bool(HttpType):
    ...
    def serialize(
        ...
        return ValueDict("Boolean", self.value).serialize()
    ...
    def _deserialize(
        ...
        if value_dict is None:
            return None
        if value_dict.type_name != "Boolean":
            return None

        v = value_dict.value
        if not isinstance(v, bool):
            value_dict.invalid_value(f"must be bool, was {type(v)}")
            return None
    ...
    def __eq__(self, other: object) -> bool:
        if type(self) is type(other):
            return self.value == t.cast(Bool, other).value
