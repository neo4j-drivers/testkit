...
class NoRoutingV4x1(TestkitTestCase):
    ...
    def _assert_is_transient_rollback_exception(self, e):
        if get_driver_name() in ["java"]:
            self.assertEqual("org.neo4j.driver.exceptions.TransientException",
                             e.errorType)
        self.assertTrue("Unable to rollback" in e.msg)
        self.assertEqual("Neo.TransientError.General.DatabaseUnavailable",
                         e.code)

    def _assert_is_transient_commit_exception(self, e,
                                              expected_msg="Unable to commit"):
        if get_driver_name() in ["java"]:
            self.assertEqual("org.neo4j.driver.exceptions.TransientException",
                             e.errorType)
