...
class Standalone:
    ...
    def start(self, network):
        ...
        if self._mtls:
            # BOLT
            env_map["NEO4J_dbms_ssl_policy_bolt_enabled"] = "true"
            env_map["NEO4J_dbms_ssl_policy_bolt_base__directory"] = \
                "/certificates/bolt"
            env_map["NEO4J_dbms_ssl_policy_bolt_private__key"] = \
                "private.key"
            env_map["NEO4J_dbms_ssl_policy_bolt_public__certificate"] = \
                "public.crt"
            env_map["NEO4J_dbms_ssl_policy_bolt_client__auth"] = \
                "REQUIRE"
            env_map["NEO4J_server_bolt_tls__level"] = "REQUIRED"

            # HTTPS
            env_map["NEO4J_server_http_enabled"] = "false"
            env_map["NEO4J_server_https_enabled"] = "true"
            env_map["NEO4J_dbms_ssl_policy_https_enabled"] = "true"
            env_map["NEO4J_dbms_ssl_policy_https_base__directory"] = \
                "/certificates/https"
            env_map["NEO4J_dbms_ssl_policy_https_private__key"] = \
                "private.key"
            env_map["NEO4J_dbms_ssl_policy_https_public__certificate"] = \
                "public.crt"
...
class Cluster:
    ...
    def start(self, network):
        for i in range(self._num_cores):
            core = Core(i, self._artifacts_path, self._version, self._mtls)
...
class Core:
    ...
    def __init__(self, index, artifacts_path, version, mtls):
    ...
    def start(self, image, initial_members, network):
        ...
        self._container = docker.run(
            image, self.name,
            env_map=env_map, network=network, mount_map=mount_map,
