...
class TestPythonLine:
    def test_python_line(self):
        ctx = EvalContext()
        content = "x = 1 + 1"
        line = PythonLine(10, "PY: " + content, content)
        line.exec(ctx)
        assert ctx["x"] == 2

    def test_failing_python_line(self):
        ctx = EvalContext()
        content = 'raise RuntimeError("oh look, a pink elephant!")'
        line = PythonLine(10, "PY: " + content, content)
        with pytest.raises(RuntimeError) as exc:
            line.exec(ctx)
        assert ("oh look, a pink elephant!",) == exc.value.args

    def test_keeps_context(self):
        ctx = EvalContext()
        content = "x = 1"
        line = PythonLine(10, "PY: " + content, content)
        line.exec(ctx)
        assert ctx["x"] == 1
        content = "x += 1"
        line = PythonLine(20, "PY: " + content, content)
        line.exec(ctx)
        assert ctx["x"] == 2
