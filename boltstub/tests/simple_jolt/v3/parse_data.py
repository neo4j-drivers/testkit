...
V3_LOADS = (
    ...
    *(
    ...
    (
        '{"V": ["i16", "FF EE"]}',
        JoltVector("i16", b"\xff\xee"),
    ),
    (
        '{"V": ["i32", "FF EE DD CC"]}',
        JoltVector("i32", b"\xff\xee\xdd\xcc"),
    ),
    (
        '{"V": ["i64", "FF EE DD CC BB AA 99 88"]}',
        JoltVector("i64", b"\xff\xee\xdd\xcc\xbb\xaa\x99\x88"),
    ),
    (
        '{"V": ["f32", "FF EE DD CC"]}',
        JoltVector("f32", b"\xff\xee\xdd\xcc"),
    ),
    (
...
def _get_v3_explicit_loads():
    res = []

    for in_, out in V3_LOADS:
        ...
        if (
            ...
            loaded_in[key + "v3"] = loaded_in.pop(key)
            res.append((json.dumps(loaded_in), out))

    return tuple(res)


V3_EXPLICIT_LOADS = _get_v3_explicit_loads()
